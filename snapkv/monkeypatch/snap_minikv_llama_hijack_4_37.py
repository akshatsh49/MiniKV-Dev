...
##############
class Snap_MKVCache(DynamicCache):
    '''
    ...
    def __init__(self, quant_bits:int = 2, group_size:int = 16, residual_length:int = 128, num_layers = 32):
        ...
        # print(f"[INFO] Using Snap_MKVCache with quant_bits: {quant_bits}, group_size: {group_size}, residual_length: {residual_length}, num_layers: {num_layers}")
    ...
    def reset(self):
        ...
        raise RuntimeError(f"Shouldn't be calling reset explicitly on Snap_MKVCache")
...
def snap_minikv_llama_flash_attn2_forward(
    ...
    **kwargs,
    ...
    if isinstance(past_key_value, DynamicCache) and not isinstance(past_key_value, Snap_MKVCache):
        raise RuntimeError(f"past_key_value should be of type Snap_MKVCache, got {type(past_key_value)}")
...
def snap_minikv_prepare_inputs_for_generation_llama(
    self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, **kwargs
):
    if past_key_values is None: # [SnapKV]
        for layer in self.model.layers:
        ...
        past_key_values = Snap_MKVCache(quant_bits = self.config.quant_bits, group_size = self.config.group_size, residual_length = self.config.residual_length, num_layers = self.config.num_hidden_layers)
